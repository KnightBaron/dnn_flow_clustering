...
OUTPUT_FILE = "/tmp/findmax/data_500hz_noised.csv.gz"
OUTPUT_FILE_NOIP = "/tmp/findmax/data_500hz_noised_noip.csv.gz"
OUTPUT_FILE_NOMETA = "/tmp/findmax/data_500hz_noised_nometa.csv.gz"
    ...
    with gzip.open(OUTPUT_FILE, "wb") as output_file,\
        ...
        for (identifier, packets) in flows.iteritems():  # Loop through each (noised) flow
            ...
            for window, counter_window in izip(windows, counter_windows):
                ...
                if point > max_value:
                    max_value = point
                point = max(counter_window)
                if point > max_counter:
                    max_counter = point

    print("DONE!")
    print("MAX PACKET SIZE IS:")
    print(max_value)
    print("MAX PACKET COUNTER IS:")
    print(max_counter)
