...
MODEL_FILE = "/work/pongsakorn-u/tensorflow_model/sigmoid/{}outputs/model-240000".format(TOTAL_CLASS)
META_GRAPH_FILE = MODEL_FILE + ".meta"

LOG_PATH = "/work/pongsakorn-u/tensorflow_classified/log/sigmoid"
OUTPUT_FILE = "/work/pongsakorn-u/tensorflow_classified/sigmoid/{}outputs.csv.gz".format(TOTAL_CLASS)
CHECKPOINT_FREQUENCY = 1000
# BATCH_SIZE = 161  # Should be divisible to total input count to use all data
BATCH_SIZE = 100  # Must be the same as in training for some reason...
...
# TOTAL_PARTS = 3108 # Full
TOTAL_PARTS = 100  # Subset
...
if __name__ == "__main__":
    ...
    with tf.Session() as session:
        ...
        with tf.name_scope("network"):
            ...
            layer3 = tf.nn.sigmoid(tf.add(tf.matmul(layer2, weights["layer3"]), biases["layer3"]), name="layer3")
            output = tf.nn.softmax(layer3)
            ...
            while not coord.should_stop():
                try:
                    ...
                    for example, probabilities in zip(example_batch, probabilities_batch):
                        ...
                        classes[predicted_class] += 1  # DEBUG
